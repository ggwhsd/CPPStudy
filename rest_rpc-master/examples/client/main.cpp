...
void test_callback() {
  ...
  std::function<void(asio::error_code, string_view)> callback1 = [](const asio::error_code& ec, string_view data) {
      ...
      std::cout << "异步 getSum= " << str << '\n';
  };
  //异步调用server端注册的 getSum 方法。底层使用的asio库的asio_write发送，以及使用asio_read接收。
  //异步调用无需等待结果，当结果返回时，会自动调用callback函数。
  client.async_call<0>("getSum", callback1, 1,2);
  //同步调用等待结果
  int sumValue=client.call<int>("getSum", 1, 2);
  std::cout << "同步 getSum= " << sumValue << '\n';
  ...
  //回调函数
  std::function<void(asio::error_code, string_view)> callback = [](const asio::error_code& ec, string_view data) {
      auto str = as<std::string>(data);
      std::cout << "echo " << str << '\n';
  };
  //异步调用server端注册的 echo 方法。底层使用的asio库的asio_write。
  client.async_call<0>("echo",callback,"xxxxx");
