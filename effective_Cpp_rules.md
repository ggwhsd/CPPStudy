...
继承，派生类继承基类，由于析构函数为`private`，所以，将`private`改为`protect`，则派生类能访问，因此能使用`heap`，但是这时，派生类中是可以创建在non heap中的。

内含，就是把某类对象作为其他类的成员，此时可以使用指针的方式，此方法也可以只允许其生成在`heap`中。

** 判断对象是否在heap中。 **

看了半天，作者的观点是，如果要有通用的判断方法，那是不存在的，要设计程序，避免判断。

** 禁止对象产生于heap中。 **

看看思路即可，目前在内存使用得当的情况下，不需要考虑这个问题，而且这本书中对这个问题也交代了，没有移植性的做法可以解决。

思路1：限制operator new和operator delete的访问权限，限制为private。 

思路2：使用计数《more effective c++》条款26，当new的时候，构造函数判断计数变量，这样就是变相的解决了这个问题

## 《more effective c++》条款28

智能指针(smart pointers)，这个在c++11中已经普遍使用了。当你使用智能指针取代c++中的内建指针（dumb pointers），将会更加安全使用。
